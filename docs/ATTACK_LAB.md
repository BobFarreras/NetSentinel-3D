...
Esta guia documenta el modulo **ATTACK LAB** (antes: **EXTERNAL AUDIT**) y su modo **LAB AUDIT** en NetSentinel 3D.
...
El backend valida (ver `src-tauri/src/application/attack_lab/validation.rs`):
...
### 5.3 Cancelacion
`cancel_attack_lab` envia una senal interna; el runner hace `child.kill()` y espera el `wait()`.
...
3. Implementa `buildRequest(ctx)` devolviendo `AttackLabRequestDTO`:
...
### 9.1 No salen logs
Comprobar:
- que el proceso realmente se ejecuta (si falla al spawn, se emitira `attack-lab-exit` con `error`),
...
3. El escenario se ejecuta desde `AttackLabPanel` y reporta por:
   - `attack-lab-log`
   - `attack-lab-exit`
...
4. `App.tsx` abre `AttackLabPanel` con:
...
## 16) Attack Lab desacoplado (ventana independiente)

Estado actual de UX:
1. `AttackLabPanel` puede ejecutarse acoplado o en ventana desacoplada.
...
- Si el operador lanza `LAB AUDIT` desde `DeviceDetailPanel` y `External` ya esta desacoplado,
  ...
  - `netsentinel://attack-lab-context`
