...
// CORRECCIÓN: Usamos border explícito para evitar conflictos de mezcla de propiedades
...
export const LabModeView: React.FC<LabModeViewProps> = ({ scenarios, selectedId, onSelect, targetDevice, selectedScenario, isRunning, onRun, onCancel, onClear }) => {
  return (
    <div style={{ display: "flex", gap: 12, padding: 12, flexShrink: 0 }}>
      <div style={{ flex: 1 }}>
        <div style={{ color: "#5c7", fontSize: 10, marginBottom: 4, textTransform: "uppercase" }}>Selected Scenario</div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {isRunning ? (
             <button 
                ...
                style={{ 
                    ...btnStyle(false), 
                    borderColor: "#00e5ff", // Sobrescribe limpiamente
                    color: "#00e5ff", 
                    opacity: 1, 
                    cursor: "wait", 
                    flex: 1 
