            ...
            {results.map((port) => {
                 ...
                 return (
                    <div key={port.port} style={{
                        ...
                        borderLeft: isDanger ? '3px solid red' : `3px solid ${HUD_COLORS.accentGreen}`,
                        background: 'rgba(0, 20, 0, 0.4)'
                    }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ color: isDanger ? '#ff4444' : HUD_COLORS.accentGreen, fontWeight: 'bold' }}>
                                PORT {port.port} <span style={{ opacity: 0.7 }}>({port.service})</span>
                            </span>
                            <span style={{
                                background: isDanger ? '#440000' : '#002200',
                                color: isDanger ? '#ffaa00' : HUD_COLORS.accentGreen,
