...
// FX de Kill Net en 3D: enjambre de drones + rayos (beams) y anillo pulsante alrededor del nodo.
...
export const JammerSwarmFx: React.FC = () => {
  ...
  const seedsRef = useRef<{ phase: number; speed: number; radius: number; yAmp: number; fireBias: number }[]>([]);
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < drones.length; i += 1) {
      ...
      const y = 0.18 + Math.sin(tt * 1.35) * s.yAmp + 0.06 * Math.sin(t * (6.0 + i * 0.9) + s.phase);
  ...
  return (
    <group ref={groupRef}>
      ...
      <mesh ref={beamBRef}>
        <cylinderGeometry args={[0.01, 0.01, 1, 6, 1, true]} />
